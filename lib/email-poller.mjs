 ...
 * Assigns invoices to users via routing rules (from/subject match → user_id).
 */

import { ImapFlow } from 'imapflow';
import { writeFileSync, readFileSync, existsSync, mkdirSync } from 'node:fs';
import { join, extname, basename } from 'node:path';
import { createInvoice, matchRoutingRules, getDb } from './db.mjs';
...
function resolveOwnerForEmail(fromAddr, subject) {
  const routedUserId = matchRoutingRules(fromAddr, subject);
  if (routedUserId) return routedUserId;
  const admin = getDb().prepare("SELECT id FROM users WHERE role = 'admin' ORDER BY id LIMIT 1").get();
...
async function processMessage(client, msg, config) {
  ...
  const ownerId = resolveOwnerForEmail(fromAddr, subject);
  ...
  if (validAttachments.length > 0) {
    saved = await downloadAttachments(client, msg, validAttachments, notePrefix, ownerId);
  } else if (/发票|invoice/i.test(subject)) {
    saved = await processLinksFromBody(client, msg, notePrefix, ownerId);
  }
...
async function downloadAttachments(client, msg, attachments, notePrefix, ownerId) {
  let saved = 0;
  for (const att of attachments) {
    try {
      ...
      const path = saveInvoiceFile(buffer, att.filename || 'attachment' + extname(att.filename || ''), ownerId);
      createInvoice({
      ...
      saved++;
      console.log(`[email-poller] Saved attachment: ${basename(path)} (${buffer.length}B) -> user #${ownerId || 'none'}`);
    } catch (err) {
...
async function processLinksFromBody(client, msg, notePrefix, ownerId) {
  let htmlBody = '';
  try {
  ...
  for (const url of links.slice(0, 5)) {
    try {
      ...
      const path = saveInvoiceFile(buffer, urlFilename, ownerId);
      createInvoice({
        invoice_type: 'other', direction: 'inbound', source: 'email',
        status: 'pending', file_path: path, notes: notePrefix,
        invoice_date: new Date().toISOString().split('T')[0], amount: 0, category: 'other'
      ...
      pollerState.processedCount++;
      saved++;
      console.log(`[email-poller] Saved from link: ${basename(path)} (${buffer.length}B) -> user #${ownerId || 'none'}`);
    } catch (err) {
...
function saveInvoiceFile(buffer, filename, ownerId) {
  const now = new Date();
  const base = ownerId ? join(INVOICES_DIR, String(ownerId)) : INVOICES_DIR;
  const destDir = join(base, now.getFullYear().toString(), String(now.getMonth() + 1).padStart(2, '0'));
