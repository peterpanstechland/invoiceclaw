...
export function batchUpdateReimbursement(ids, status) {
  if (!ids?.length) return 0;
  const db = getDb();
  const placeholders = ids.map(() => '?').join(',');
  const reimbursedAt = status === 'reimbursed' ? new Date().toISOString() : null;
  const result = db.prepare(
    `UPDATE invoices SET reimbursement_status = ?, reimbursed_at = ?, updated_at = datetime('now')
     WHERE id IN (${placeholders})`
  ).run(status, reimbursedAt, ...ids);
